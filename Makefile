...
test:
	pytest -rsx --verbose --color=yes --cov=openeo_flood_mapper_local --cov-report term-missing

version:
	echo -e "__version__ = \"$(shell git describe --always --tags --abbrev=0)\"\n__commit__ = \"$(shell git rev-parse --short HEAD)\"" > src/openeo_flood_mapper_local/_version.py
