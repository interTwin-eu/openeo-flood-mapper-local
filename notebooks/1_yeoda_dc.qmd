...
This approach is realised in openEO with the parent function `apply_neighborhood()` with a window size of 5 by 5 pixels. The child process `median` is used to average the window's values.

```{python}
flood_decision = flood_dc. \
    apply_neighborhood("median", dict(x=5, y=5)). \
    execute()
```
